...
const SecureLink = React.forwardRef<HTMLAnchorElement, React.ComponentPropsWithoutRef<"a">>(
  ({ className, href, target, rel, children, ...props }, ref) => {
    const { safeHref, finalRel } = React.useMemo(() => {
      ...
      // We check against the original url, not safe, to ensure even blocked protocol-relative URLs
      // get the secure rel attributes if they somehow render or bypass checks in edge cases.
